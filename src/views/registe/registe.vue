<template>
  <div class="page" :style="{height: flag ? '50%' : '100%'}">
    <div class="registe-form" :style="{top: flag ? '-210px' : ''}">
      ...
      <div class="row-wrapper register-wrapper">
        <div type="button" class="submit" :class="{bgColor:bgColor}" @click.stop="registe()" v-if="!flag">注册并登录</div>
        <button type="button" class="submit" :class="{bgColor:bgColor}" @click.stop="registe()" v-else>注册并登录</button>
      </div>
      <div class="registe-link">
        <span class="txt" @click="()=>{this.$router.replace({path: '/login'})}">使用密码登录</span>
...
export default {
  ...
  methods: {
    ...
    registe() {
      ...
      if (this.wxObj !== {}) {
        obj.openid = this.wxObj.openid;
        obj.unionId = this.wxObj.unionId;
      }
      console.log(obj);
      axois.post('/api/user/register', obj).then((res) => {
        if (res.data && res.data.code === 0) {
          console.log('注册成功!开始自动登陆!');
          Toast({
            message: '注册成功!自动登录中',
            position: 'middle',
            duration: 1500
    ...
    WX() {
      ...
      // 获取openid和unionId
        wxjs.init(code).then(rs => {
          if (rs.wxconfig.openid && rs.wxconfig.unionid) {
            _self.wxObj.openid = rs.wxconfig.openid;
            _self.wxObj.unionId = rs.wxconfig.unionid;
            console.log(_self.wxObj);
      ...
      // 跳到第三方页面获取code值
        if (/MicroMessenger/.test(window.navigator.userAgent)) {
          window.location.replace(`${process.env.WEB_HOST}/registe/entry`);
        }
      }
    }
    // sin() {
