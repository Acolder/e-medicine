<template>
  <div class="chat-box " v-bind:class="[direction]">
    <!-- 头像-->
    <div class="head-portrait" v-if="type!=='app:endConsult' && type !== 'app:evaluate'">
      <img :src="imgUrl" alt="">
      <div class="name" v-if="name"> {{name}}</div>
    </div>
    <!--内容-->
    <div class="speech" v-if="type!=='app:endConsult' && type !== 'app:evaluate'">
      <div class="angel"></div>
      <div class="info" :class="{'info-padding': type === 'app:evaluate' || type === 'app:doctorPresc', 'default-width': type === 'app:PrescriptionProtocol'}">
        ...
        <!-- 如果是问诊评价 -->
        <!-- <div v-if="type === 'app:evaluate'" class="evaluate-box">
